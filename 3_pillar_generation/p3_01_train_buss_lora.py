...
OUTPUT_DIR = "buss_lora_model_FINAL" 
NUM_EPOCHS = 3
LEARNING_RATE = 5e-5
BATCH_SIZE = 4 
BIPOLAR_WEIGHT = 0.01 # Lambda (λ)
MAX_LENGTH = 128 
MAX_NEW_TOKENS = 50 

# --- NEW: Define target scores based on p2_05 results ---
# 'great' was -0.24, 'love' was -0.20
TARGET_POS_PROJECTION = -0.20
# 'bad' was +0.37, 'worst' was +0.12
TARGET_NEG_PROJECTION = 0.30


# --- CUDA Device Setup ---
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"--- Starting Pillar 3 (FINAL Corrected): BUSS-LORA Training ---")
...
# ===================================================================
# STEP 6: Training Loop (Corrected Loss)
# ===================================================================
...
for epoch in range(NUM_EPOCHS):
    ...
    for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}"):
        ...
        # Obtener la proyección BUSS del texto GENERADO
        generated_projections = get_buss_projection(generated_text) # Shape: [batch_size, 1]

        # ---  Create target projections based on labels ---
        # 1.0 (pos) -> TARGET_POS_PROJECTION (-0.2)
        # 0.0 (neg) -> TARGET_NEG_PROJECTION ( 0.3)
        ...
        total_loss = lm_loss + (buss_loss * BIPOLAR_WEIGHT)
    ...
    print(f"  Average BUSS Loss: {avg_buss_loss:.4f}")
...
print("\n--- Pillar 3 Training Complete (FINAL) ---")