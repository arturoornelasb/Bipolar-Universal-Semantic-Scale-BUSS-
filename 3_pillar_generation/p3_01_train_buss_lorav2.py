...
# --- V2 EXPERIMENT: Increase Bipolar Weight ---
BIPOLAR_WEIGHT = 0.1 # <-- x10  ( 0.01)
OUTPUT_DIR = "buss_lora_model_lambda_0_1" 
# -----------------------------------------------
...
print(f"--- Starting Pillar 3 (V2 - Lambda=0.1): BUSS-LORA Training ---")
...
for epoch in range(NUM_EPOCHS):
    ...
    for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}"):
        ...
        total_loss = lm_loss + (buss_loss * BIPOLAR_WEIGHT) # <-- BIPOLAR_WEIGHT = 0.1
    ...
    print(f"  Average BUSS Loss: {avg_buss_loss:.4f}") # <-- ¡Vigila este número!
