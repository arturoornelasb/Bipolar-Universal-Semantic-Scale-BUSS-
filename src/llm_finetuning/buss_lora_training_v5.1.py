...
Este script implementa el 'approach' estable v2.8 (prefijo + Tanh Loss)
pero reemplaza el DEMO_DATASET de demostración con el
...
bipolares (positivos vs. negativos) y previene el sobreajuste.
"""
...
def load_imdb_data(base_dir, max_samples_per_class=1000):
    ...
    del dataset aclImdb. Limita los tokens por eficiencia.
    """
    texts_standard = []
    texts_opposite = []
    ...
    # Mezclamos para obtener una muestra aleatoria
    np.random.shuffle(pos_files) 
    
    for file_path in pos_files[:max_samples_per_class]:
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                # Leemos la crítica, limpiamos y limitamos a 64 tokens
                text = f.read()
                text = re.sub(r'<br\s*/?>', ' ', text) # Quitar <br />
                text = re.sub(r'\s+', ' ', text).strip()
                tokens = text.split()
                texts_standard.append(' '.join(tokens[:64]))
    ...
    for file_path in neg_files[:max_samples_per_class]:
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                texts_opposite.append(f"Bipolar_Opposite: {' '.join(tokens[:64])}")
        except Exception as e:
            print(f"Advertencia: No se pudo leer {file_path}: {e}")
...
def train_buss_lora_imdb():
    ...
    # Usamos max_length=128 para capturar más de la crítica
    dataset = BipolarDataset(texts_standard, texts_opposite, tokenizer, max_length=128) 
