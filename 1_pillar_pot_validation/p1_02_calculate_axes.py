...
print("--- Starting Pillar 1 (Steps 3, 4, & 5) ---")
...
# Load E from disk
# ===================================================================
print("Loading sparse matrix E from disk...")
E = load_npz(os.path.join(output_dir, "E.npz"))
print(f"Matrix E loaded. Shape: {E.shape}")

# ===================================================================
# STEP 3: Create Matrix Ec (Centered E) --- (THE DENSE FIX)
# ===================================================================
...
# This is the step that uses RAM (50000x5000 = ~2GB)
E_dense = E.toarray()

# 2. Now, center the DENSE matrix.
# We will use StandardScaler for consistency.
scaler = StandardScaler(with_std=False) # with_std=False only subtracts mean
E_c = scaler.fit_transform(E_dense)
# E_c is now a DENSE (50000, 5000) centered matrix

# (Optional: free up memory immediately)
del E_dense 
...
print("(This is the BUSS calculation and may take a minute...)")
start_time = time.time()

# TruncatedSVD is smart: it will run a dense SVD algorithm
# because its input (E_c) is now a dense numpy array.
...
# TruncatedSVD is smart: it will run a sparse SVD algorithm
# because its input (E) is still a sparse matrix.
...
np.save(os.path.join(output_dir, "V_buss.npy"), V_buss)
np.save(os.path.join(output_dir, "V_lsa.npy"), V_lsa)

print(f"Successfully saved V_buss.npy and V_lsa.npy to '{output_dir}'")

print("\n--- Steps 3, 4, and 5 completed! ---")